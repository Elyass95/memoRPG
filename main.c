...
void jouer1(infojoueur *a, infojoueur *b, infojoueur *s, infojoueur *d, int r,
  ...
  *r4 = 0; // initialisation du nombres de cartes révélées du joueur
  ...
  while (1) {
    ...
    if(nbj>=1){
    while (c == 1 ||
           c == 2) { // Si c = 1 ou c = 2 cela veut dire que le joueur à avancer
                     // dans le tour précédent ce qui lui permet donc de pouvoir
                     // rejouer d'où cette boucle while.
      printf("-------------------------------------------------------------");
      ...
      sleep(1);
      cha = choisir_arme();
      choix = position(xj1, yj1);
      revelation(tableau, cartes, xj1, yj1, choix, xr, yr, r1);
      sleep(1);
      c = condition(tableau, cartes, cha, choix, xj1, yj1, xr, yr, m1, e1, t1,
                    1);
      ...
      if (c == 1 || c==2) { // Si c =1 c'est que le joueur peut avancer dans la
                    // direction qu'il choisi.

        changement(tableau, cartes, xj1, yj1,choix); // on échange la case du joueur par la case où il doit aller.
        if(c==2){
          changementprotail(tableau, cartes, xj1, yj1);
        }
        sleep(1);
        cvictoire(cartes, 1, xj1, yj1, e1,t1); // récupération si besoins de l'équipement du joueur ou
                       // d'un trésor.
        ...
        if (*e1 == 1 && *t1 == 1) {
          ...
          ecriture(a, b, s, d, m1, m2, m3, m4, r1, r2, r3, r4, t1, t2, t3, t4,
                   e1, e2, e3, e4,1, fichier); // on écrit toutes les données de
          ...
          printf(
              "The program was executed in %f seconds.\n",
              tempsExecution *
                  100000); // on calcule et affiche le temps de jeu en seconde.
          int rm;
          printf("[1] replay \n\n");
          printf("[2] main menu ");
          do {
          ...
          if (rm == 1) {
            jouer1(a, b, s, d, 1, nbj,fichier); // 1 on rentre dans la condition pour rejoueur
          } else if (rm == 2) {
            menu(fichier); // 2 on rentre dans la condition pour retourner au
                           // menu
          }
        } // Cette condition permet de savoir si le joueur à gagné, lorsqu'il a
          // gagné on rentre dans la condition
    ...
    while (c == 1 || c == 2) {
      ...
      sleep(1);
      cha = choisir_arme();
      choix = position(xj2, yj2);
      revelation(tableau, cartes, xj2, yj2, choix, xr, yr, r2);
      sleep(1);
      c = condition(tableau, cartes, cha, choix, xj2, yj2, xr, yr, m2, e2, t2,
                    2);
      // condition pour savoir si on peut avancer ou pas
      sleep(1);
    ...
    while (c == 1 || c == 2) {
      ...
      sleep(1);
      cha = choisir_arme();
      choix = position(xj3, yj3);
      revelation(tableau, cartes, xj3, yj3, choix, xr, yr, r3);
      sleep(1);
      c = condition(tableau, cartes, cha, choix, xj3, yj3, xr, yr, m3, e3, t3,
                    3);
      // condition pour savoir si on peut avancer ou pas
      sleep(1);
...
if(nbj>=4){
    ...
    while (c == 1 || c == 2) {
      ...
      sleep(1);
      cha = choisir_arme();
      choix = position(xj4, yj4);
      revelation(tableau, cartes, xj4, yj4, choix, xr, yr, r4);
      sleep(1);
      c = condition(tableau, cartes, cha, choix, xj4, yj4, xr, yr, m4, e4, t4,
                    4);
      // condition pour savoir si on peut avancer ou pas
      sleep(1);
